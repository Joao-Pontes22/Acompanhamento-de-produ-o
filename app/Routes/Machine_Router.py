...
@Machine_Router.get("/GET_machine_by_id", response_model=Response_Machines)
...
@Machine_Router.get("/GET_machine_by_name", response_model=Response_Machines)
...
async def updatte_machine(id:int, scheme:Update_Machine_Scheme, session:Session=Depends(Init_Session)):
    ...
    service = Service_Machines(machine_repo=repo)
    try:
        new_machines_info = service.service_update_machine_info(id=id, scheme=scheme, sectors_repo=sectors_repo)
